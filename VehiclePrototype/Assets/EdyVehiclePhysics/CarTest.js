...
function Update ()
	...
	if (Input.GetKeyDown(KeyCode.Alpha8))
		{
		// Detener y posicionar el veh�culo en el punto de partida
		
		m_Car.GetComponent.<Rigidbody>().AddForce(-m_Car.GetComponent.<Rigidbody>().velocity, ForceMode.VelocityChange);
		m_Car.GetComponent.<Rigidbody>().AddTorque(-m_Car.GetComponent.<Rigidbody>().angularVelocity, ForceMode.VelocityChange);
		m_Car.GetComponent.<Rigidbody>().MovePosition(StartPoint.position);
		m_Car.GetComponent.<Rigidbody>().MoveRotation(StartPoint.rotation);
		
		// Detener grabaci�n
		
		StopRecording();
		
		// Reiniciar los valores m�ximos
	...
	if (Input.GetKeyDown(KeyCode.Alpha9))
		{
		m_Car.GetComponent.<Rigidbody>().AddForce(SideTestVelocity * m_Car.transform.right, ForceMode.VelocityChange);
	...
	// Regenerar el gr�fico de los valores y mostrarlo
...
private function SetServiceMode(Car : CarControl, serviceMode : boolean, freezeRotation : boolean)
	...
	Car.GetComponent.<Rigidbody>().freezeRotation = freezeRotation;
	}
...
function FixedUpdate ()
	...
	m_speed = Mathf.Abs(Vector3.Dot(m_Car.GetComponent.<Rigidbody>().velocity, m_Car.transform.forward));
	m_accel = Mathf.Abs((m_speed-m_lastSpeed) / Time.deltaTime);
	m_speedLat = Mathf.Abs(Vector3.Dot(m_Car.GetComponent.<Rigidbody>().velocity, m_Car.transform.right));
	...
	if (m_bRecording)
		...
		if (m_recValues.Count > 3)
			...
			// Localizar y calcular el valor de slip en 1.0 cuando el slip pasa de m�s de 1.0 a menos de 1.0;
...
function StopRecording ()
	...
	// Finalizar grabaci�n y guardar datos
	...
	// Actualizar el gr�fico si est� visible

...
function Start ()
	{
	// Preparar los gr�ficos de las curvas de fricci�n
	...
	// Preparar los gr�ficos de las barras estabilizadoras
	...
	// Preparar los gr�ficos de la suspensi�n
	...
	// Gr�ficos de las barras de valor
...
function DoWheelGizmos(Wheel : CarWheel)
	...
	if (bGrounded)
		...
		if (Velocity) Debug.DrawLine(Hit.point, Hit.point + Target.GetComponent.<Rigidbody>().GetPointVelocity(Hit.point), Color.blue, 0.0, false);
		}
	}
...
function DoTelemetry ()
	...
	var SpeedMS = Vector3.Dot(Target.GetComponent.<Rigidbody>().velocity, Target.transform.forward);
	var SpeedLatMS = Vector3.Dot(Target.GetComponent.<Rigidbody>().velocity, Target.transform.right);
	...
	var AngV = Target.GetComponent.<Rigidbody>().angularVelocity;
	...
	m_sDescription += String.Format("\nSpeed:{0,6:0.00} m/s {1,5:0.0} km/h {2,5:0.0} mph\n  Abs:{3,6:0.00} m/s Lat:{4,5:0.00} m/s\n  Acc:{5,5:0.00} m/s2 Lat:{6,5:0.00} m/s2\nPitch:{7,6:0.00} Roll:{8,6:0.00}  Max:{9,6:0.00}\n",
						SpeedMS, SpeedMS*3.6, SpeedMS*2.237, Target.GetComponent.<Rigidbody>().velocity.magnitude, SpeedLatMS, (SpeedMS-m_LastSpeedMS) / Time.deltaTime, (SpeedLatMS-m_LastSpeedLatMS) / Time.deltaTime, pitch, roll, Target.getMaxRollAngle());
	...
	// Telemetr�a del script de control
	...
	// Telemetr�a de las configuraciones
...
function DrawSuspensionPerformance(x : int, y : int, graph : GUICanvas, WheelCol : WheelCollider, travel : float)
	...
	travel = 1.0 - travel; // Representar compresi�n: 0.0 = extendido, 1.0 = comprimido
